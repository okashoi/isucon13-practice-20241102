...
func DNSHandler(w dns.ResponseWriter, r *dns.Msg) {
	...
	case dns.TypeNS:
		if strings.ToLower(question.Name) == "u.isucon.dev." {
			m.Answer = []dns.RR{
				newRR("u.isucon.dev. 120 IN NS ns1.u.isucon.dev."),
			}
			m.Extra = []dns.RR{
				newRR("ns1.u.isucon.dev. 120 IN A 192.168.0.11"),
			}
		}
	...
	case dns.TypeA:
		...
		if isPresent {
			...
			// m.Ns = []dns.RR{
			// 	newRR("u.isucon.dev. 60 IN SOA ns1.u.isucon.dev. hostmaster.u.isucon.dev. 2023100201 10800 3600 604800 3600"),
			// }
			// ここでは無回答にしています。
		}
